...
var arr =[
  ...
  function clicking(e){
    ...
    console.log("clicked")
    var data_get = parseInt(e.getAttribute("for"))-1
    var data_next = arr[data_get]
    console.log(arr[data_get])
    var final_data = document.createElement('div')
    final_data.innerHTML =
    `
         <p for=${data_get} > ${data_next.id}</p>
         <p for="name" > ${data_next.name}</p>
         <p for="email"> ${data_next.email}</p>
 
    `
    document.querySelector('.visual').append(final_data )
...
next_button.addEventListener('click',()=>{
    ...
    var userdata = arr.length;
    var value_out = (parseInt(userdata/5))*5
  
    for(i=1; i<=6; i++){
       
  
   var first_button   = document.querySelector(`.col-${i}`)

   var final_button = parseInt(first_button.getAttribute('for'))
   
   if(value_out>=final_button){
   
   var third_final = final_button + 5
   
   first_button.setAttribute('for',third_final);
  
    first_button.innerText =  third_final
...
var next_button = document.querySelector(".previous_page")
next_button.addEventListener('click',()=>{
